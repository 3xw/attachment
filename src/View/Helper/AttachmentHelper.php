...
class AttachmentHelper extends Helper
{
  ...
  private function _getSettings($field,$settings)
  {
    ...
    $settings['baseUrls'] = [];
    $settings['thumbBaseUrls'] = [];
    foreach(Configure::read('Attachment.profiles') as $key => $value)
    {
      $settings['baseUrls'][$key] = $value['baseUrl'];
      if($cdn = Configure::read('Attachment.profiles.'.$key.'.thumbnails.cdn')) $settings['thumbBaseUrls'][$key] = $cdn->getUrl();
      else $settings['thumbBaseUrls'][$key] = $this->Url->build('/thumbnails/');
    }
    return $settings;
  }
  ...
  {
    $this->_setupInputComponent();
    $conf['relation'] = (preg_match('~^\p{Lu}~u', $field))? 'belongsToMany' : 'belongsTo';
    $conf['field'] = strtolower($field);
  ...
  public function thumbSrc($params) {
    ...
    $cdn = Configure::read('Attachment.profiles.'.$profile.'.thumbnails.cdn');
